...
class PdfToPptWindow(ctk.CTkToplevel):
    def __init__(self, master, ortalama_func, engine, success_dialog_class, access_controller=None):
        ...
        self.title(t("pdf_to_ppt.window_title"))
        self.ortalama_func(self, 640, 600)
        self.grab_set()
        self.configure(fg_color=self.ui["bg"])

        build_tool_header(self, t("pdf_to_ppt.header"), t("pdf_to_ppt.detail"))
    ...
    def run_convert(self):
        save_path = filedialog.asksaveasfilename(parent=self, title=t("pdf_to_ppt.save_title"), defaultextension=".pptx", filetypes=[("PowerPoint", "*.pptx")])
        if not save_path:
            return
        ...
        progress_dialog = ProgressDialog(self, self.ortalama_func, total_count=2, title=t("pdf_to_ppt.progress_title"))
        progress_dialog.update_progress(0, 2, t("progress.starting"))

        def worker():
            try:
                if self.access_controller:
                    self.access_controller.authorize_operation("pdf-to-ppt", [self.selected_file])
                import pdf_toolkit_extra
                pdf_toolkit_extra.pdf_to_pptx(self.selected_file, save_path, password=self.selected_password)
