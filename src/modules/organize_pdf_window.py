...
class OrganizePdfWindow(ctk.CTkToplevel):
    def __init__(self, master, ortalama_func, engine, success_dialog_class, access_controller=None):
        ...
        self.title(t("organize_pdf.window_title"))
        self.ortalama_func(self, 820, 780)
        self.grab_set()
        self.configure(fg_color=self.ui["bg"])
        self.protocol("WM_DELETE_WINDOW", self._on_close)

        build_tool_header(self, t("organize_pdf.header"), t("organize_pdf.detail"))
    ...
    def _on_order_changed(self, order: list):
        pass  # order değişimi butonla tetiklenir, sadece data tutulur

    def run_organize(self):
        if not self.visual_grid:
            return
        new_order = self.visual_grid.get_order()

        save_path = filedialog.asksaveasfilename(
            parent=self, title=t("organize_pdf.save_title"),
        ...
        progress_dialog = ProgressDialog(self, self.ortalama_func, total_count=2,
                                         title=t("organize_pdf.progress_title"))
        progress_dialog.update_progress(0, 2, t("progress.starting"))

        def worker():
            try:
                if self.access_controller:
                    self.access_controller.authorize_operation("organize-pdf", [self.selected_file])
                import pdf_toolkit_extra
                pdf_toolkit_extra.organize_pdf(
                    self.selected_file, save_path, new_order,
