...
class FlattenPdfWindow(ctk.CTkToplevel):
    def __init__(self, master, ortalama_func, engine, success_dialog_class, access_controller=None):
        ...
        self.title(t("flatten_pdf.window_title"))
        self.ortalama_func(self, 640, 580)
        self.grab_set()
        self.configure(fg_color=self.ui["bg"])

        build_tool_header(self, t("flatten_pdf.header"), t("flatten_pdf.detail"))
    ...
    def run_flatten(self):
        save_path = filedialog.asksaveasfilename(parent=self, title=t("flatten_pdf.save_title"), defaultextension=".pdf", filetypes=[("PDF", "*.pdf")])
        if not save_path:
            return
        ...
        progress_dialog = ProgressDialog(self, self.ortalama_func, total_count=2, title=t("flatten_pdf.progress_title"))
        progress_dialog.update_progress(0, 2, t("progress.starting"))

        def worker():
            try:
                if self.access_controller:
                    self.access_controller.authorize_operation("flatten-pdf", [self.selected_file])
                import pdf_toolkit_extra
                pdf_toolkit_extra.flatten_pdf(self.selected_file, save_path, password=self.selected_password)
