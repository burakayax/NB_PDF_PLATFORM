...
/**
 * Sitede kaç araç olduğu — TEK KAYNAK.
 *
 * Bu sayı ana sayfa başlığı, H1, açıklama ve fiyat kartında geçiyordu ve HER
 * BİRİNE ELLE yazılmıştı. Yeni araç eklendiğinde güncellenmesi unutuldu: site
 * 46 araç sunarken Google'a ve ziyaretçiye "45 araç" diyordu. Artık listeden
 * türetiliyor, kendiliğinden doğru kalıyor.
 */
export const ARAC_SAYISI = TOOL_SLUGS.length;
...
export const TOOL_SEO = {
  "gorsel-sikistir": {
    tr: T(
      "Görsel Sıkıştırma — Fotoğraf Boyutu KB Düşürme",
      "Fotoğrafı 50, 100 veya 200 KB'a düşürün. Hedef boyutu yazın, kaliteyi biz ayarlayalım — görseliniz cihazınızdan çıkmaz, üyeliksiz ve ücretsiz.",
      "Fotoğraf Boyutu Küçültme — KB ve MB Düşürme",
      "Bir kurum «en fazla 100 KB» diyorsa kalite yüzdesiyle uğraşmanıza gerek yok: hedef boyutu yazın, o sınırın altında kalan en yüksek kaliteyi biz buluruz. 20 KB, 50 KB, 100 KB, 200 KB, 500 KB ve 1 MB için hazır düğmeler var; dilediğiniz rakamı da elle girebilirsiniz. Kaç kalite kullanıldığını ve gerekirse ölçünün küçültüldüğünü sonuç ekranında açıkça yazarız — sürpriz olmaz. Boyut düşürmek için görseli KIRPMAYIN; kırpmak görüntünün bir kısmını kaybettirir. Her şey tarayıcınızda çalışır, görseliniz sunucuya yüklenmez.",
      ["fotoğraf boyutu küçültme", "resim boyutu küçültme kb", "resim kb düşürme", "fotoğraf kb düşürme", "100 kb resim yapma", "50 kb fotoğraf", "resim mb düşürme", "görsel sıkıştırma", "jpg sıkıştırma", "online resim sıkıştırıcı"],
      [
        { q: "Fotoğrafı 100 KB'a nasıl düşürürüm?", a: "Görseli yükleyin, «Hedef boyuta göre» seçeneğine geçin ve 100 KB düğmesine basın. Araç, 100 KB'ın altında kalan en yüksek kaliteyi kendisi bulur; kaç kalite kullandığını da size söyler." },
        { q: "Hangi hedef boyutlar hazır geliyor?", a: "20 KB, 50 KB, 100 KB, 200 KB, 500 KB ve 1 MB tek tıkla seçilebilir. Bunların dışında bir rakam gerekiyorsa KB cinsinden elle yazabilirsiniz." },
        { q: "Kalite ne kadar düşer?", a: "Hedefe ulaşmak için kullanılan kalite sonuç ekranında yüzde olarak yazar ve ne anlama geldiği düz Türkçe açıklanır. %80 üzeri kayıp gözle fark edilmez; %45 altında yazılar ve ince ayrıntılar bozulmaya başlar." },
        { q: "Hedefe inilemezse ne olur?", a: "Size bunu açıkça söyleriz ve elde edilen en küçük hâli veririz — sessizce hedefin üstünde bir dosya vermeyiz. Böyle bir durumda ölçüyü düşürmeyi ya da hedefi biraz yükseltmeyi deneyin." },
        { q: "Boyutu düşürmek için fotoğrafı kırpmalı mıyım?", a: "Hayır. Kırpmak görüntünün bir kısmını kalıcı olarak atar ve çoğu zaman gereksizdir. Hedef boyut modu görüntünün tamamını koruyarak dosyayı küçültür." },
        { q: "Görselim sunucuya yüklenir mi?", a: "Hayır. Sıkıştırma tamamen tarayıcınızda (cihazınızda) çalışır; görseliniz internete hiç gönderilmez, üyelik de gerekmez." },
      ],
    ),
    en: T(
      "Reduce Image Size in KB — 50, 100, 200 KB Targets",
      "Shrink a photo to 50, 100 or 200 KB. Type the target size and we pick the quality — your image never leaves your device, no signup, free.",
      "Reduce Image Size to a Target in KB or MB",
      "When a form says «100 KB maximum», you should not have to guess at quality percentages: type the target size and we find the highest quality that stays under it. There are one-click targets for 20 KB, 50 KB, 100 KB, 200 KB, 500 KB and 1 MB, and you can enter any figure yourself. We state plainly on the result screen which quality was used, and whether the dimensions had to be reduced — no surprises. Do not CROP to hit a size limit; cropping throws away part of the picture. Everything runs in your browser and your image is never uploaded.",
      ["reduce image size in kb", "compress image to 100kb", "reduce photo size kb", "image compressor", "compress image to target size", "reduce image size", "compress jpg", "online image compressor"],
      [
        { q: "How do I reduce a photo to 100 KB?", a: "Upload the image, switch to «To a target size» and click the 100 KB button. The tool finds the highest quality that stays under 100 KB and tells you what that quality turned out to be." },
        { q: "Which target sizes are built in?", a: "20 KB, 50 KB, 100 KB, 200 KB, 500 KB and 1 MB are one click away. For anything else, type the figure in KB." },
        { q: "How much quality is lost?", a: "The quality used to reach your target is shown as a percentage on the result screen, with a plain-language note on what it means. Above 80% the loss is invisible; below 45% text and fine detail start to degrade." },
        { q: "What if the target cannot be reached?", a: "We say so plainly and give you the smallest version achieved — we never quietly hand back a file above your target. When that happens, try lowering the dimensions or raising the target slightly." },
        { q: "Should I crop the photo to make it smaller?", a: "No. Cropping permanently discards part of the picture and is usually unnecessary. Target-size mode shrinks the file while keeping the whole image." },
        { q: "Is my image uploaded to a server?", a: "No. Compression runs entirely in your browser (on your device); your image is never sent to the internet and no account is needed." },
      ],
    ),
  },
  "gorsel-boyutlandir": {
  ...
  "pdf-kesit-al": {
    tr: T(
      ...
      "PDF sayfasında istediğiniz alanı seçip PDF, PNG ya da JPEG olarak kaydedin. Tablo, grafik ya da şemayı kesip alın — belgeniz cihazınızdan çıkmaz.",
      "PDF'ten Kesit Alma",
      "Bir PDF sayfasının istediğiniz bölgesini seçip yüksek çözünürlüklü görsel olarak dışa aktarın: tablo, grafik, şema, logo ya da tek bir soru. Ekran görüntüsünden farklı olarak kesit doğrudan belgeden, seçtiğiniz çözünürlükte (1x/2x/3x) çizilir; bu yüzden çok daha nettir. Aldığınız kesitler bir sepette birikir ve sırasını değiştirebilirsiniz. Kaydederken biçimi siz seçersiniz: PDF (varsayılan) kesitleri tek belgede toplar — ister A4 sayfalara sırayla dizilmiş, yazdırmaya hazır bir çalışma kâğıdı (tek veya iki sütun), ister her kesit ayrı sayfa. PNG ya da JPEG seçerseniz kesitler görsel olarak iner; birden çoksa ZIP içinde gelir. Tüm işlem tarayıcınızda çalışır; belgeniz sunucuya yüklenmez, üyelik ve kurulum gerekmez.",
      ...
      [
        ...
        { q: "Birden fazla kesiti tek dosyada alabilir miyim?", a: "Evet, varsayılan davranış budur. PDF biçiminde kesitler tek belgede toplanır: 'hepsi arka arkaya' derseniz A4 sayfalara sırayla dizilip yazdırmaya hazır bir çalışma kâğıdı olur (tek veya iki sütun), 'her kesit ayrı sayfa' derseniz her biri kendi sayfasında durur." },
        { q: "Hangi biçimlerde kaydedebilirim?", a: "PDF (varsayılan; kesitleri tek belgede toplar), PNG (metin ve grafikler için keskin, kayıpsız) ya da JPEG (fotoğraflar için daha küçük dosya)." },
    ...
    en: T(
      ...
      "Select any area of a PDF page and save it as PDF, PNG or JPEG. Grab a table, chart or diagram — your document never leaves your device.",
      "Snip PDF to Image",
      "Export any region of a PDF page as a high-resolution image: a table, chart, diagram, logo or a single question. Unlike a screenshot, the snip is rendered straight from the document at the resolution you choose (1x/2x/3x), so it is far sharper. Your snips collect in a basket and you can reorder them. You pick the format when you save: PDF (the default) collects them into one document — either a print-ready A4 worksheet with the snips laid out in order (one or two columns), or one snip per page. Choose PNG or JPEG instead and the snips download as images, in a ZIP when there is more than one. Everything runs in your browser; your document is never uploaded, and no signup or installation is needed.",
      ...
      [
        ...
        { q: "Can I get several snips in one file?", a: "Yes, that is the default. In PDF format the snips are collected into one document: 'all one after another' lays them out in order on A4 pages as a print-ready worksheet (one or two columns), while 'one snip per page' keeps each on its own page." },
        { q: "Which formats can I save in?", a: "PDF (the default; collects the snips into one document), PNG (lossless and sharp for text and graphics) or JPEG (a smaller file, better for photos)." },
...
export const LANDING_SEO = {
  tr: {
     ...
     */
    title: markaEkle(`PDF Birleştirme, Word'e Çevirme ve Küçültme — ${ARAC_SAYISI} Araç`),
    description:
    ...
    h1: `PDF Birleştir, Word'e Çevir, Küçült ve Düzenle — ${ARAC_SAYISI} PDF Aracı Tek Platformda`,
    intro:
      `PDF Platform ${ARAC_SAYISI} araç sunar: PDF birleştirme, ayırma, küçültme (sıkıştırma), Word/Excel/PowerPoint ve JPG dönüştürme, düzenleme, imzalama, form doldurma, tarama, filigran ve şifreleme. Birleştirme ve görselden PDF gibi araçlar üyelik gerektirmeden, tamamen tarayıcınızda çalışır — dosyalarınız cihazınızdan hiç çıkmaz, anında ve 80 MB'a kadar ücretsizdir. Kurulum gerekmez.`,
    faq: [
  ...
  en: {
    ...
    title: markaEkle(`PDF to Word, Merge & Compress — ${ARAC_SAYISI} PDF Tools`),
    description:
...
export const RELATED_TOOLS = {
  ...
  "rotate-pdf": ["organize-pdf", "delete-pages", "split-pdf"],
  "organize-pdf": ["delete-pages", "rotate-pdf", "split-pdf"],
  "pdf-kesit-al": ["pdf-to-image", "extract-images", "gorsel-boyutlandir"],
  ...
  "gorsel-boyutlandir": ["gorsel-sikistir", "image-to-pdf", "pdf-to-image"],
...
export const BLOG_RELATED_TOOLS = {
  ...
  "pdf-kirpma-kenar-boslugu-kesme": ["pdf-kesit-al", "rotate-pdf", "organize-pdf"],
