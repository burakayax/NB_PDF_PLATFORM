// ─────────────────────────────────────────────────────────────────────────────
// YASAL METİNLER — TEK GERÇEK KAYNAK
// Hem React (LegalPage / CookieNotice) hem Node üretim betiği
// (generate-seo-files.mjs) bu dosyayı okur. Betik .ts okuyamadığı için içerik
// burada .mjs olarak durur; tipler legal.ts tarafında tanımlıdır.
//
// Neden: şartlar/gizlilik/KVKK sayfaları önceden üretilen HTML'de yalnızca
// başlık + tek cümle içeriyordu (15-20 kelime). Arama motoru bu sayfaları
// neredeyse boş görüyordu. Artık tam metin de üretiliyor.
// ─────────────────────────────────────────────────────────────────────────────
      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "Tool ratings and feedback: when you rate a tool, your star rating and (for low ratings) the short comment you write are stored. This record is not linked to your identity: your name, email or account details are not stored. To stop the same person rating the same tool repeatedly, an irreversible signature is derived from your IP address and browser information; the raw IP is not stored. As a result we cannot trace a rating back to the person who left it, and for the same reason a request to delete one specific rating cannot be fulfilled. Please do not type personal information into the comment box.",
        ...
        {
          title: "6a. Commercial electronic messages (promotional email)",
          paragraphs: [
            "We send promotional email — campaigns, discounts, product news and similar — only to users who have explicitly agreed to receive it. Consent is collected through a separate, entirely optional checkbox during sign-up; it is unchecked by default, and declining it does not restrict your use of the service in any way.",
            "As proof of your consent we record the date, the channel it came through, the exact text shown to you at the time, and your connection details. This record exists to meet our burden of proof under Turkish Law No. 6563 and its implementing regulation, and is not used for any other purpose.",
            "To stop receiving promotional email, click the unsubscribe link at the bottom of any such message; your request is processed immediately and no further action is needed from you. Once you unsubscribe, we do not send you promotional email again.",
            "Transactional email is not covered by this: email verification, password resets, security alerts, invoices and receipts, subscription and billing notices, signature requests and support replies are part of how the service operates and continue even after you unsubscribe from promotional email. These messages contain no promotion or purchase inducement.",
          ],
        },
        {
          title: "7. Security",
      ...
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            ...
            "| nb_tool_ratings_v1 | Hangi araca puan verdiğinizi hatırlar (aynı soruyu tekrar sormamak için) | PDF Platform (birinci taraf) | Süresiz (localStorage) |",
        ...
        {
          title: "6a. Ticari elektronik iletiler (tanıtım e-postaları)",
          paragraphs: [
            "Kampanya, indirim, yenilik duyurusu ve benzeri tanıtım içeren e-postaları yalnızca bunu açıkça kabul eden kullanıcılara göndeririz. Onay, kayıt sırasında işaretlenmesi tamamen isteğe bağlı olan ayrı bir kutu ile alınır; varsayılan olarak işaretli değildir ve onay vermemek hizmetin kullanımını hiçbir şekilde kısıtlamaz.",
            "Onayınızın kanıtı olarak onayın tarihi, alındığı kanal, o sırada size gösterilen metin ve bağlantı bilgileriniz kaydedilir. Bu kayıt, 6563 sayılı Kanun ve ilgili Yönetmelik kapsamındaki ispat yükümlülüğümüzü yerine getirmek için tutulur ve başka bir amaçla kullanılmaz.",
            "Tanıtım e-postalarından çıkmak için e-postanın altındaki bağlantıya tıklamanız yeterlidir; talebiniz anında işlenir, ayrıca bir işlem yapmanız gerekmez. Çıkış yaptıktan sonra size tanıtım e-postası gönderilmez.",
            "İşlem e-postaları bu kapsamda değildir: e-posta doğrulama, parola sıfırlama, güvenlik uyarısı, fatura ve makbuz, abonelik ve tahsilat bildirimleri, imza talepleri ve destek yanıtları hizmetin işleyişinin parçasıdır ve tanıtım aboneliğinden çıksanız da gönderilmeye devam eder. Bu e-postaların içinde tanıtım veya satın almaya özendirme yer almaz.",
          ],
        },
        {
          title: "7. Güvenlik",
