...
import { groupToolsByCategory, toolCategoryLabel } from "../../i18n/workspace";
...
export function ToolsGrid({
  ...
  // Araçları kategori bölümlerine ayır (mobil/sidebar ile aynı gruplama).
  const byId = new Map(tools.map((t) => [t.id, t] as const));
  const groups = groupToolsByCategory(tools.map((t) => t.id));
        ...
        {groups.map((group) => {
          ...
          return (
            <section key={group.id}>
              <h3 className="mb-2.5 flex items-center gap-2 text-[12px] font-semibold uppercase tracking-[0.12em] text-nb-muted lg:text-[13px]">
                <span className="h-1.5 w-1.5 rounded-full bg-nb-primary/60" />
                {label}
                <span className="text-nb-muted/50">({groupTools.length})</span>
              </h3>
