...
export function DashboardRecentActivity({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-nb-panel/60 p-4 lg:p-5 2xl:p-6">
      <h2 className="mb-3 text-sm font-semibold text-nb-heading sm:text-base lg:text-lg">
        ...
        <>
          ...
          <div className="divide-y divide-white/[0.06] md:hidden">
            {entries.map((entry) => (
              <div key={entry.id} className="flex items-center gap-3 py-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-nb-bg-elevated">
                  <TypeDot type={entry.type} />
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-xs font-medium text-nb-text">
                    {L.creditTxToolLabel(entry.toolLabel)}
                  </p>
                  <p className="text-[10px] text-nb-muted">{formatDate(entry.createdAt)}</p>
                </div>
                <span
          ...
          <div className="hidden md:block">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs font-medium uppercase tracking-wider text-nb-muted">
                  <th className="pb-2 pr-4">{tr ? "Araç" : "Tool"}</th>
                  <th className="pb-2 pr-4">{tr ? "Tür" : "Type"}</th>
                  <th className="hidden pb-2 pr-4 lg:table-cell">
                    {tr ? "Tarih" : "Date"}
                  </th>
                  <th className="pb-2 text-right">{tr ? "Miktar" : "Amount"}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.06]">
                {entries.map((entry) => (
                  <tr key={entry.id} className="text-nb-text">
                    ...
                    <td className="py-2.5 pr-4 text-xs text-nb-muted">
                      {typeLabel(entry.type)}
                    </td>
                    <td className="hidden py-2.5 pr-4 text-xs text-nb-muted lg:table-cell">
                      {formatDate(entry.createdAt)}
                    </td>
                    <td
                      className={`py-2.5 text-right text-xs font-semibold tabular-nums ${
                        entry.type === "consume"
                          ? "text-rose-300"
                          : entry.type === "bonus"
                          ? "text-emerald-400"
